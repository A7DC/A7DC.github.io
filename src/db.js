const data = {
  ...
  work: [
    { title: 'UI Work', subtitle: 'Dribbble', bg: 'images/dribbble-bg.png', padding: 'pr4'},
    { title: 'Medium.com', subtitle: 'Writing', bg: 'images/medium-bg.png', padding: 'pl4'}
  ],
  workExperience: [
  ...
  caseStudy: [
    {
      bordellio: {
        features: [
          { title: 'Browse listings', content: 'Bordellio finds you the nearest escorts to you, showing the highest rated ones first. A button at the bottom of the page allows you to filter.' },
          ...
          { title: 'Booking a date', content: 'Booking a date is simple. Once you’ve filtered your requirements, simply select any profile and tap on the ‘book now’ button. This will send a message request to your date.If the advertiser accepts this request you’ll be notified and able to communicate directly with them. In the meantime you’re able to leave the app!' },
          { title: 'Messaging', content: 'Messaging between advertiser and client is completely encrypted using the Signal API. It is here the two parties are able to discuss the details of their date, as well as send / request identification and / or deposits.' },
          { title: 'Requesting ID (optional)', content: 'Advertisers are now able to request ID. Simply select the ‘request ID’ button and an automated request will be sent to the client. The client will then be able to securely send an image to the advertiser, verifying who they are.Because of the encrypted messaging.' },
          { title: 'Self-destructing messaging', content: 'Message requests (once accepted) will expire after 1 hour unless both parties have confirmed the booking, after which the messages will self-destruct 12 hours after the date has completed. We hope this will decrease the likelihood of failed bookings.If your date is a no-show then give them a negative rating, explain the reason, and we’ll work towards making sure it doesn’t happen again.' },
          { title: 'Confirming date', content: 'After the advertiser has accepted the message request, they will then have to confirm the date request itself. Once done, the client will be able to confirm the date!' },
          { title: 'Declining a date', content: 'After the advertiser has accepted the message request, both parties will have the option to decline the date request. Once done, the conversation will instantly self-destruct.Neither party will be rated.' },
